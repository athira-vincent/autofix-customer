...
class _MechanicStartServiceScreenState extends State<MechanicStartServiceScreen>
  ...
  Future<void> getSharedPrefData() async {
    ...
    await _firestore
      ...
      setState(() {
        ...
        selectedServiceTime = Duration(
                minutes:
                    int.parse('${allData[0]['serviceTime'].split(".").first}'))
            .inSeconds;
  ...
  void listenToCloudFirestoreDB() {
    DocumentReference reference =
        FirebaseFirestore.instance.collection('ResolMech').doc('${bookingId}');
    reference.snapshots().listen((querySnapshot) {
      setState(() {
        customerDiagonsisApproval =
            querySnapshot.get("customerDiagonsisApproval");
        oldSelectedServiceId = querySnapshot.get("serviceId");

        totalservicetimereturn =
    ...
    /// api calling for remainder time

    // Repository().timedifferenceapi(currenttime, totalservicetimereturn).then((value) => {
    //
  ...
  void updateToCloudFirestoreDB() {
    ...
    if (allData.isNotEmpty) {
      _firestore
          ...
          .catchError(
              (error) => print("Failed to add updatedServiceList: $error"));
    ...
    return InkWell(
      ...
      child: Container(
        decoration: BoxDecoration(
            ...
            border: Border.all(color: CustColors.light_navy, width: 0.3)),
        margin: EdgeInsets.only(
            left: size.width * 4 / 100,
    ...
    return InkWell(
      onTap: () async {
       ...
       await Repository()
        ...
        /// here we upload  total service time + current time  to firebase
      ...
      child: Align(
        ...
        child: Container(
          margin: EdgeInsets.only(
              right: size.width * 4 / 100, top: size.height * 5.2 / 100),
          decoration: BoxDecoration(
              borderRadius: BorderRadius.all(
                Radius.circular(6),
              ),
          ...
          child: Text(
            startOrUpdateState == "0" ? "Start services" : "Update Services",
    ...
    if (serviceList!.isNotEmpty) {
      ...
      setState(() {
        ...
        for (int i = 0; i < serviceList!.length; i++) {
          if (serviceList[i].service?.serviceName.toString() !=
              selectedServiceName) {
            allData.add({
              'isDefault': '0',
              'serviceId': '${serviceList[i].id.toString()}',
              'serviceName':
                  '${serviceList[i].service?.serviceName.toString()}',
              'serviceCost': '${serviceList[i].fee.toString()}',
              'serviceTime':
                  '${serviceList[i].time.split(":").first.toString()}'
            });
          }
        }
        print(
            " allData >>>>>>>> _awaitReturnValueFromSecondScreenOnAdd  $allData");

        for (int i = 0; i < allData.length; i++) {
          print('${allData[i]['isDefault']} >>>>>>>>>> isDefault');
          if (allData[i]['isDefault'] == "0") {
            additionalServiceNames = additionalServiceNames +
                allData[i]['serviceName'].toString() +
                " \n";
            ...
            selectedServiceTime = Duration(
                    minutes: int.parse(
                        '${serviceTotalTimeForFirebase.split(":").first}'))
                .inSeconds;
            print('selectedServiceTime >>>>>>>>>> $selectedServiceTime');
            print(
                'serviceTotalTimeForFirebase >>>>>>>>>> $serviceTotalTimeForFirebase'); //total Time of Additional srvices and default service added
            print(
                'serviceTotalCostForFirebase >>>>>>>>>> $serviceTotalCostForFirebase'); //total Amount of Additional srvices and default service added
          }
        }

        print("serviceTotalTimeForFirebase2233  >>>>> " +
            serviceTotalTimeForFirebase);
        print("selectedServiceTime .............." +
            selectedServiceTime.toString());
        selectedServiceTime =
            Duration(minutes: int.parse('${serviceTotalTimeForFirebase}'))
                .inSeconds;
        levelClock = selectedServiceTime;
        _controller = AnimationController(
            vsync: this, duration: Duration(seconds: selectedServiceTime));
        _controller.reset();
      });
    }
  ...
  void _awaitReturnValueFromSecondScreenOnChange(BuildContext context) async {
    ...
    if (result.isNotEmpty) {
      setState(() {
        ...
        print(" allData _awaitReturnValueFromSecondScreenOnChange>>>>>>> " +
            allData.toString());

        setState(() {
          selectedServiceName = '${result[0].service!.serviceName.toString()}';
          serviceTotalTimeForFirebase =
              int.parse('${result[0].time.split(":").first}').toString();
          serviceTotalCostForFirebase = result[0].fee.toString();
          print(" serviceTotalTimeForFirebase2 >>>>>>> " +
              serviceTotalTimeForFirebase);
          print(" serviceTotalCostForFirebase2 >>>>>>> " +
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        Container(
                          child: SvgPicture.asset(
                            isWaiting == "-1"
                                ? "assets/image/img_oops_customer_bg.svg"
  ...
  Widget tryAgainButtonWidget(Size size) {
    return Align(
      ...
      child: Container(
        margin: EdgeInsets.only(
            right: size.width * 2.2 / 100, top: size.height * 3 / 100),
        decoration: BoxDecoration(
            borderRadius: BorderRadius.all(
              Radius.circular(6),
            ),
